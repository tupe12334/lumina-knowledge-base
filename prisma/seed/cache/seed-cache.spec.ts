...
vi.mock('../../../generated/client', async () => {
  const actual = (await vi.importActual('../../../generated/client')) as unknown as typeof client
...
describe('SeedCache', () => {
  it('returns cached translation without hitting database again', async () => {
    const t = await prisma.translation.create({ data: { en_text: 't', he_text: 't' } })
  ...
  it('returns cached university', async () => {
    const translation = await prisma.translation.create({ data: { en_text: 'u', he_text: 'u' } })
    const uni = await prisma.university.create({ data: { id: 'u1', translationId: translation.id, createdAt: new Date(), updatedAt: new Date() } })
